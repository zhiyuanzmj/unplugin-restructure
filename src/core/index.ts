...
import type { OptionsResolved, Prop } from './options'
import type {
...
export const transformRestructure = (
  ...
  const s = new MagicStringAST(code, { filename: id })
  const ast = babelParse(code, 'tsx', { sourceFilename: id })
  walkAST<Node>(ast, {
    enter(node) {
      if (
        ...
        node.leadingComments?.find(
          (comment) => comment.value === ' @restructure ',
        )
      ) {
